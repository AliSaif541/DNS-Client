...
var DnsClient = /** @class */ (function () {
    ...
    DnsClient.prototype.queryFlow = function (index, headerID, name, type) {
        if (type === "A") {
            var packet = PacketInfo_1.DNSPacket.makeAQuery(headerID, name);
            StartingPoint_1.sockets.performDnsQuery(packet.toBuffer());
        }
        else if (type === "AAAA") {
            var packet = PacketInfo_1.DNSPacket.makeAAAAQuery(headerID, name);
            StartingPoint_1.sockets.performDnsQuery(packet.toBuffer());
        }
        else if (type === "CNAME") {
            var packet = PacketInfo_1.DNSPacket.makeCNAMEQuery(headerID, name);
            StartingPoint_1.sockets.performDnsQuery(packet.toBuffer());
        }
    };
            ...
            return __generator(this, function (_b) {
                try {
                    _loop_1 = function (name_1, type) {
                        ...
                        this_1.queryFlow(index, randomNumber, realName, type);
                    };
                    this_1 = this;
